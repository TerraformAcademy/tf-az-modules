...
variable "backend_address_pool" {
  type = set(object({
    ...
    ip_addresses = optional(list(string), []) # (Optional) A list of IP Addresses which should be part of the Backend Address Pool.
  }))
...
variable "backend_http_settings" {
  type = set(object({
    ...
    affinity_cookie_name                = optional(string, null) # (Optional) The name of the affinity cookie.
    name                                = string                 # (Required) The name of the Backend HTTP Settings Collection.
    path                                = optional(string)       # (Optional) The Path which should be used as a prefix for all HTTP requests.
    ...
    request_timeout                     = optional(number, 30)   # (Optional) The request timeout in seconds, which must be between 1 and 86400 seconds. Defaults to 30.
    ...
    pick_host_name_from_backend_address = optional(bool, false)  # (Optional) Whether host header should be picked from the host name of the backend server. Defaults to false.
...
variable "frontend_ip_configuration" {
  type = set(object({
    ...
    subnet_id                       = optional(string)            # (Optional) The ID of the Subnet.
    private_ip_address              = optional(string)            # (Optional) The Private IP Address to use for the Application Gateway.
    public_ip_address_id            = optional(string)            # (Optional) The ID of a Public IP Address which the Application Gateway should use. The allocation method for the Public IP Address depends on the sku of this Application Gateway.
    private_ip_address_allocation   = optional(string, "Dynamic") # (Optional) The Allocation Method for the Private IP Address.
    private_link_configuration_name = optional(string)            # (Optional) The name of the private link configuration to use for this frontend IP configuration.
  }))
...
variable "http_listener" {
  type = set(object({
    ...
    require_sni                    = optional(bool, false)  # (Optional) Should Server Name Indication be Required? Defaults to false.
    ssl_certificate_name           = optional(string)       # (Optional) The name of the associated SSL Certificate which should be used for this HTTP Listener.
    ...
    firewall_policy_id = optional(string) # (Optional) The ID of the Web Application Firewall Policy which should be used for this HTTP Listener.
...
variable "request_routing_rule" {
  type = set(object({
    ...
    backend_address_pool_name   = optional(string) # (Optional) The Name of the Backend Address Pool which should be used for this Routing Rule. Cannot be set if redirect_configuration_name is set.
    backend_http_settings_name  = optional(string) # (Optional) The Name of the Backend HTTP Settings Collection which should be used for this Routing Rule. Cannot be set if redirect_configuration_name is set.
    redirect_configuration_name = optional(string) # (Optional) The Name of the Redirect Configuration which should be used for this Routing Rule. Cannot be set if either backend_address_pool_name or backend_http_settings_name is set.
    rewrite_rule_set_name       = optional(string) # (Optional) The Name of the Rewrite Rule Set which should be used for this Routing Rule. Only valid for v2 SKUs.
    ...
    priority                    = optional(number) # (Optional) Rule evaluation order can be dictated by specifying an integer value from 1 to 20000 with 1 being the highest priority and 20000 being the lowest priority.
  }))
...
variable "sku" {
  type = object({
    ...
    capacity = number # (Optional) The Capacity of the SKU to use for this Application Gateway. When using a V1 SKU this value must be between 1 and 32, and 1 to 125 for a V2 SKU. This property is optional if autoscale_configuration is set.
  })
...
variable "identity" {
  type = set(object({
    ...
    identity_ids = list(string) #(Required) Specifies a list of User Assigned Managed Identity IDs to be assigned to this Application Gateway.
  }))
...
variable "ssl_profile" {
  type = set(object({
    ...
    trusted_client_certificate_names     = optional(string, null)   #(Optional) The name of the Trusted Client Certificate that will be used to authenticate requests from clients.
...
variable "trusted_root_certificate" {
  type = set(object({
    ...
    data                = optional(string, null) #(Optional) The contents of the Trusted Root Certificate which should be used.
...
variable "redirect_configuration" {
  type = set(object({
    ...
    include_path         = optional(bool, false)  #(Optional) Whether to include the path in the redirected URL. 
...
variable "create_waf" {
  type        = bool
  description = "(Optional) Whether to create a web application firewall or not."
  default     = false
}
...
variable "waf_name" {
  ...
  default     = null # Set deafult to null to avoid required even when create_waf is false. 
}
